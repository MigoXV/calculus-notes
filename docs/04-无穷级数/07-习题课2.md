...
定义：如果 $\boldsymbol{f}(\boldsymbol{x})$ 为奇函数，Fourier级数 $\sum_{n=1}^{\infty} \boldsymbol{b}_{n} \boldsymbol{\operatorname { s i n }} \boldsymbol{n} \boldsymbol{x}$称为正弦级数。
...
$F(z)$ 在 $[-\pi, \pi]$ 上满足收玫定理的条件，它在每一点都连续，它的Fourier级数在 $[-\pi, \pi]$ 上收玫于 $F(z)$ ．
...
$F(z)$ 在 $(-5,5)$ 上满足收敛定理的条件，它在每一点都连续，它的Fourier级数在 $(-5,5)$ 上收玫于 $F(z)$ ．
