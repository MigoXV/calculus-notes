...
（1）当 $\rho<1$ 时，取 $\varepsilon<1-\rho$ ，使 $r=\varepsilon+\rho<1$ ， $\boldsymbol{u}_{N+2}<r \boldsymbol{u}_{N+1}, \boldsymbol{u}_{N+3}<r \boldsymbol{u}_{N+2}<r^{2} \boldsymbol{u}_{N+1}, \cdots$, $\boldsymbol{u}_{N+m}<\boldsymbol{r}^{m-1} \boldsymbol{u}_{N+1}$ ，而级数 $\sum_{m=1}^{\infty} r^{m-1} \boldsymbol{u}_{N+1}$ 收玫， $\therefore \sum_{m=1}^{\infty} \boldsymbol{u}_{N+m}=\sum_{n=N+1}^{\infty} \boldsymbol{u}_{u}$ 收玫。
（2）当 $\rho>1$ 时，取 $\varepsilon<\rho-1$ ，使 $r=\rho-\varepsilon>1$ ，当 $n>N$ 时，$u_{n+1}>r u_{n}>u_{n}, \lim _{n \rightarrow \infty} u_{n} \neq 0$ ．故级数发散．
（3）$\sum_{n=1}^{\infty} \frac{1}{n}$ 发散，但 $\lim _{n \rightarrow \infty} \frac{u_{n+1}}{u_{n}}=\lim _{n \rightarrow \infty} \frac{n}{n+1}=1$ ； $\sum_{n=1}^{\infty} \frac{1}{n^{2}}$ 收玫，但 $\lim _{n \rightarrow \infty} \frac{u_{n+1}}{u_{n}}=\lim _{n \rightarrow \infty} \frac{n^{2}}{(n+1)^{2}}=1$ ．
...
解（1）$\because \lim _{n \rightarrow \infty} \frac{u_{n+1}}{u_{n}}=\lim _{n \rightarrow \infty} \frac{n!}{(n+1)!}=\lim _{n \rightarrow \infty} \frac{1}{n+1}=0 \quad<1$ ，由正项级数的比值审敛法知原级数 $\sum_{n=1}^{\infty} \frac{1}{n!}$ 收玫。
...
比值审玫法失效，改用比较审玫法
$\because \frac{1}{(2 n-1) \cdot 2 n}<\frac{1}{n^{2}}$ ，而级数 $\sum_{n=1}^{\infty} \frac{1}{n^{2}}$ 收玫，

由正项级数的比较审玫法知原级数 $\sum_{n=1}^{\infty} \frac{1}{2 n \cdot(2 n-1)}$ 收玫。
说明：比值审玫法一般适用于级数通项
...
而级数 $\sum_{n=1}^{\infty} \frac{1}{n^{2}}$ 收玫，由正项级数的比较法知原级数收玫。
...
$\lim _{n \rightarrow \infty} \sqrt[n]{u_{n}}=\lim _{n \rightarrow \infty} \frac{\sqrt[n]{3+(-1)^{n}}}{2}=\frac{1}{2}<1$ ，所以原级数收玫。
或 $\frac{3+(-1)^{n}}{2^{n}} \leq \frac{4}{2^{n}}$ ，且 $\sum_{n=1}^{\infty} \frac{4}{2^{n}}$ 收玫，所以原级数收敛。

上述判别正项级数玫散性的四个审敛法，都是充分条件，如果用其中的某一个审敛法不能判定所给级数的玫散性，那么就要改用其它的审敛法以及级数收玫与发散的定义，收敛的性质、必要条件等去判别，这需要学生通过练习不断总结各种方法优劣及适用范围，熟练而灵活地使用它们。
（28）（14）（№）（3）
...
（ i ）$u_{n} \geq u_{n+1}(n=1,2,3, \cdots)$ ；（ii） $\lim _{n \rightarrow \infty} u_{n}=0$ ，则级数收玫，且其和 $s \leq u_{1}$ ，其余项 $r_{n}$ 的绝对值 $\left|\boldsymbol{r}_{\boldsymbol{n}}\right| \leq \boldsymbol{u}_{\boldsymbol{n + 1}}$.
...
满足收玫的两个条件，$\quad \therefore\left|\boldsymbol{r}_{n}\right| \leq \boldsymbol{u}_{n+1}$ ．

例 3 判别级数 $\sum_{n=2}^{\infty} \frac{(-1)^{n} \sqrt{n}}{n-1}$ 的收玫性．
...
且 $v_{n} \leq\left|u_{n}\right|, \therefore \sum_{n=1}^{\infty} v_{n}$ 收玫，
又 $\sum_{n=1}^{\infty} u_{n}=\sum_{n=1}^{\infty}\left(2 v_{n}-\left|u_{n}\right|\right), \therefore \sum_{n=1}^{\infty} u_{n}$ 收玫。
...
注意：（1）若 $\sum_{n=1}^{\infty}\left|u_{n}\right|$ 收玫，则 $\sum_{n=1}^{\infty} u_{n}$ 收敛；
...
$\therefore \sum_{n=1}^{\infty} \frac{n!}{n^{n}}$ 收玫，故原级数绝对收玫。
...
$\therefore \sum_{n=1}^{\infty} \frac{|\alpha|^{n}}{n!}$ 收玫，故原级数绝对收玫。
（3）$\because\left|\frac{\sin n}{n^{2}}\right| \leq \frac{1}{n^{2}}$ ，而 $\sum_{n=1}^{\infty} \frac{1}{n^{2}}$ 收玫，
...
由莱布尼兹判别法可知，该交错级数收玫。故原级数是条件收敛，不是绝对收敛的．
...
5．利用级数收玫的必要条件可证明： $\lim _{n \rightarrow \infty} u_{n}=0$ ．
...
（2）$\because \sum_{n=1}^{\infty} u_{n}=\sum_{n=1}^{\infty}\left[\frac{1}{(2 n-1)^{2}}-\left(\frac{1}{7}\right)^{2 n}\right]$而 $\sum_{n=1}^{\infty} \frac{1}{(2 n-1)^{2}}$ 与 $\sum_{n=1}^{\infty}\left(\frac{1}{7}\right)^{2 n}$ 都收玫。所以原级数收玫。但显然不满足 $u_{n} \geq u_{n+1}$ ．
...
易得 $u_{n}>u_{n+1}, ~ \lim _{n \rightarrow \infty} u_{n}=\lim _{n \rightarrow \infty} \frac{4 n}{(n+1)^{2}}=0$ ，故原级数条件收玫．

1．利用正项级数审敛法

2．利用绝对收敛定理和莱布尼兹判别法讨论任意项级数的绝对收敛与条件收敛

对任意项级数，先用正项级数的审玫法判断它是否绝对收玫，否则，再用莱布尼兹审敛法判断它是否收敛，若收敛，则知它是条件收敛。
...
故级数 $\sum_{n=1}^{\infty} \frac{1}{n!}$ 收玫。
...
故级数 $\sum_{n=1}^{\infty} \frac{1}{2 n \cdot(2 n-1)}$ 收玫。
（3）当 $\rho=1$ 时，比值法与根值法都失效 ，须用比较法或比较法的 极限形式。
所以原级数收玫。
...
所以原级数发散．
（3）$\because \lim _{n \rightarrow \infty} \frac{u_{n}}{v_{n}}=\lim _{n \rightarrow \infty} \frac{\ln \left(1+\frac{1}{n^{2}}\right)}{\frac{1}{n^{2}}}=1>0$ ，而级数 $\sum_{n=1}^{\infty} \frac{1}{n^{2}}$ 收玫，所以原级数收玫。
（4）$\because \lim _{n \rightarrow \infty} \frac{u_{n+1}^{n=1}}{u_{n}}=\frac{1}{2} \lim _{n \rightarrow \infty} \frac{3+(-1)^{n+1}}{3+(-1)^{n}}$ 不存在，
或 $\frac{3+(-1)^{n}}{2^{n}} \leq \frac{4}{2^{n}}$ ，且 $\sum_{n=1}^{\infty} \frac{4}{2^{n}}$ 收玫，所以原级数收玫。
## 二、交错级数及其审敛法
解
...
## 绝对收敛定理的作用：
$\therefore \sum_{n=1}^{\infty} \frac{n!}{n^{n}}$ 收玫，故原级数收玫。
$\therefore \sum_{n=1}^{\infty} \frac{\mid \alpha^{n}}{n!}$ 收玫，故原级数收玫。
$\therefore \sum_{n=1}^{\infty}\left|\frac{\sin n}{n^{2}}\right|$ 收玫，故由定理知原级数收玫。
2．绝对收敛与条件收敛：
若 $\sum_{n=1}^{\infty}\left|u_{n}\right|$ 收敛，则称 $\sum_{n=1}^{\infty} u_{n}$ 为绝对收敛；
若 $\sum_{n=1}^{\infty}\left|u_{n}\right|$ 发散，而 $\sum_{n=1}^{\infty} u_{n}$ 收玫，则称 $\sum_{n=1}^{\infty} u_{n}$ 为条件收敛。
注意：（1）若 $\sum_{n=1}^{\infty}\left|\boldsymbol{u}_{n}\right|$ 收玫，则 $\sum_{n=1}^{\infty} \boldsymbol{u}_{n}$ 收敛；
...
而 $\sum_{n=1}^{\infty} \frac{1}{(2 n-1)^{2}}$ 与 $\sum_{n=1}^{\infty}\left(\frac{1}{7}\right)^{2 n}$ 都收玫。所以原级数收玫。但显然不满足 $u_{n} \geq u_{n+1}$ ．
...
$\lim _{n \rightarrow \infty} u_{n}=\lim _{n \rightarrow \infty} \frac{1}{\sqrt{2 n+1}}=0$ ，故原级数条件收玫。
$(5) \sum_{n=1}^{\infty}(-1)^{n}\left[1-\frac{(n-1)^{2}}{(n+1)^{2}}\right]$ ．
例8 判别下列级数的玫散性：
（1）$\sqrt{2}+\sqrt{2-\sqrt{2}}+\sqrt{2-\sqrt{2+\sqrt{2}}}+\cdots+\sqrt{2-\sqrt{2+\cdots+\sqrt{2}}}$ ；
...
由比较法知 $\sum_{n=1}^{\infty} \int_{0}^{\frac{1}{n}} \frac{\sqrt{x}}{1+x^{2}} d x$ 收玫
例8 判别下列级数的玫散性：
\text { (3) } 1-\frac{1}{2^{\alpha}}+\frac{1}{3}-\frac{1}{4^{\alpha}}+\frac{1}{5}-\frac{1}{6^{\alpha}}+\cdots \text {; }
解 当 $\alpha=1$ 时，
1-\frac{1}{2^{\alpha}}+\frac{1}{3}-\frac{1}{4^{\alpha}}+\frac{1}{5}-\frac{1}{6^{\alpha}}+\cdots=\sum_{n=1}^{\infty}(-1)^{n-1} \frac{1}{n} \text {, 条件收玫. }
当 $\alpha>1$ 时，$\sum_{n=1}^{\infty} \frac{1}{(2 n)^{\alpha}}$ 收玫，
