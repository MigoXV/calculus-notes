...
则称曲线积分 $\int_{L} P d x+Q d y$ 在 $G$ 内与路径无关，否则与路径有关。（如 $\mathrm{P}_{222}$
...
例8 计算 $\int_{L}\left(x^{2}+2 x y\right) d x+\left(x^{2}+y^{4}\right) d y$ ．其中 $L$ 为由点 $O(0,0)$ 到点 $B(1,1)$ 的曲线弧 $y=\sin \frac{\pi x}{2}$ ．

例9 设曲线积分 $\int_{L} x y^{2} d x+y \varphi(x) d y$ 与路径无关，其中 $\varphi$ 具有连续的导数，且 $\varphi(\mathbf{0})=\mathbf{0}$ ，计算 $\int_{(0,0)}^{(1,1)} x y^{2} d x+y \varphi(x) d y$ 。
