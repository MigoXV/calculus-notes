...
例1 计算 $\iint x y d \sigma, D$ 由直线 $y=1, x=2$ 及 $y=x$ 围成。

例2 计算 $\iint_{D}^{D} x y d \sigma, D$ 由 $y^{2}=x$ 和 $y=x-2$ 围成。
...
例7 计算 $\iiint_{|x|+y \mid \leq 1}|x y| d x d y$ ．

例8 计算 $\iint_{\substack{-1 \leq x \leq 1 \\ 0 \leq y \leq 1}}\left|y-x^{2}\right| d x d y$ ．

例9 求 $I=\iint_{D} \frac{f(x)}{f(x)+f(y)} d x d y,\left(f(x)>0, D: x^{2}+y^{2}<R^{2}\right)$ ．
