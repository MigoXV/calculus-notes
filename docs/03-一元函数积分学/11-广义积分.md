...
若式中的极限存在，则称此无穷积分在柯西主值意义下收玫，极限值即为柯西主值意义下的无穷积分值；若式中极限不存在，则称该无穷积分在柯西主值意义下发散。
...
注意：对反常积分，只有在收玫的条件下才能使用 ＂偶倍奇零＂的性质，否则会出现错误。
...
例5 证明无穷积分 $\int_{a}^{+\infty} e^{-p x} d x$ 当 $p>0$ 时收玫，当 $p<0$ 时发散．
...
若式中的极限存在，则称此瑕积分在柯西主值意义下收玫。极限值即为柯西主值意义下的瑕积分值；若式中极限不存在，则称该瑕积分在柯西主值意义下发散。
