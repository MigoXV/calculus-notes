...
例6计算 $\int x \cos x d x$ 。例7计算 $\int x^{2} e^{x} d x$ 。

例 8 计算 $\int x \arctan x d x$ ．例9计算 $\int\left(x^{3}+3 x+1\right) \ln x d x$ 。

例10计算 $\int \boldsymbol{e}^{\boldsymbol{x}} \boldsymbol{\operatorname { s i n }} \boldsymbol{x} \boldsymbol{d} \boldsymbol{x}$ ．例11计算 $\boldsymbol{\int} \sqrt{\boldsymbol{x}^{\mathbf{2}}+\boldsymbol{a}^{\mathbf{2}}} \mathbf{d} \boldsymbol{x}$

例12 计算 $\int(\arcsin x)^{2} d x$ ．例13 计算 $\int \sin (\ln x) d x$ ．

例14计算 $\int \sec ^{3} x d x$.
例15计算 $\int \frac{x \arctan x}{\sqrt{1+x^{2}}} d x$ ．
例16计算 $\int\left(\ln \ln x+\frac{1}{\ln x}\right) d x$ ．例17 计算 $\int \frac{\ln x}{(1+\ln x)^{2}} d x$ ．
...
例9 计算 $\int\left(\boldsymbol{x}^{\mathbf{3}} \boldsymbol{+} \mathbf{3} \boldsymbol{x}+\mathbf{1}\right) \boldsymbol{\operatorname { l n }} \boldsymbol{x} \boldsymbol{d} \boldsymbol{x}$ ．
...
解 原式 $=\int \frac{x \ln x}{x(1+\ln x)^{2}} d x=\int \boldsymbol{x} \ln \boldsymbol{x} \boldsymbol{d}\left(-\frac{\mathbf{1}}{\mathbf{1}+\boldsymbol{\operatorname { l n }} \boldsymbol{x}}\right)$
...
## 3.1 不定积分
## 一、第二换元积分法
设 $x=\psi(t)$ 是单调的、可导的函数，并且 $\psi^{\prime}(t) \neq 0$ ，又设 $f[\psi(t)] \psi^{\prime}(t)$ 具有原函数，则有换元公式 $\int f(x) d x=\left[\int f[\psi(t)] \psi^{\prime}(t) d t\right]_{t=\bar{\psi}(x)}$其中 $\psi(x)$ 是 $x=\psi(t)$ 的反函数．
& \xlongequal{t=\overline{\psi(x)}} \Phi[\overline{\psi(x)}]+C .
## 第二换元积分法习例
例1计算 $\int \frac{\boldsymbol{x}^{5}}{\sqrt{1+\boldsymbol{x}^{2}}} d \boldsymbol{x}$
## 积分中为了消去根式并不一定采用三角
=\ln \left|\frac{\sqrt{1+e^{x}}}{\sqrt{1+e^{x}}}+1\right|+C=2 \ln \left|\sqrt{1+e^{x}}-1\right|-x+C .
## 当分母的阶较高时，可采用倒代换 $x=\frac{1{t}$ 。}
## 二、不定积分的分部积分法
\int x e^{x} d x=\int x d e^{x}=\int d x e^{x}-\int e^{x} d x=x e^{x}-e^{x}+C
## 分部积分法习例
例7 计算 $\int \boldsymbol{x}^{2} \boldsymbol{e}^{\boldsymbol{x}} \boldsymbol{d x}$ ．
## 解题技巧：
反：反三角函数
对：对数函数
幂：幂函数
指：指数函数
三：三角函数
方法1 $\int(\arcsin x)^{\mathbf{2}} d x=x(\arcsin x)^{\mathbf{2}}-\int x d(\arcsin x)^{\mathbf{2}}$
& \int \sin (\ln x) d x=x \sin (\ln x)-\int x d[\sin (\ln x)] \\
& \quad=x \sin (\ln x)-\int x \cos (\ln x) \cdot \frac{1}{x} d x \\
& \quad=x \sin (\ln x)-\int \cos (\ln x) d x \\
& \quad=x \sin (\ln x)-x \cos (\ln x)+\int x d[\cos (\ln x)] \\
& \quad=x[\sin (\ln x)-\cos (\ln x)]-\int \sin (\ln x) d x \\
& =\frac{1}{2} \sec x \tan x+\frac{1}{2} \ln \sec x+\tan x+C .
& \therefore \quad \int \frac{x \arctan x}{\sqrt{1+x^{2}}} d x=\int \arctan x d \sqrt{1+x^{2}} \\
& =\sqrt{1+x^{2}} \arctan x-\ln \left(x+\sqrt{1+x^{2}}\right)+C
解 原式 $=\int \frac{x \ln x}{x(1+\ln x)^{2}} d x=\int x \ln x d\left(-\frac{1}{1+\ln x}\right)$
## 利用分部积分法可以得到某些积分的递推公式
## 小结：
