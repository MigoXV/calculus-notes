...
例1 计算 $\int_{0}^{\frac{\pi}{2}} \cos ^{5} x \sin x d x$ ．例2 计算 $\int_{0}^{\pi} \sqrt{\sin ^{3} x-\sin ^{5} x} d x$ ．
例3 计算 $\int_{\sqrt{e}}^{e^{\frac{3}{4}}} \frac{\boldsymbol{d x}}{\boldsymbol{x} \sqrt{\ln \boldsymbol{x}(\mathbf{1}-\boldsymbol{\operatorname { l n }} \boldsymbol{x})}}$ ．例4 计算 $\int_{0}^{\frac{\pi}{2}} \frac{\cos x}{\sin x+\cos x} d x$ ．
例5 计算 $\int_{0}^{4} \frac{d x}{1+\sqrt{x}}$ ．例6 计算 $\int_{0}^{a} \frac{1}{x+\sqrt{a^{2}-x^{2}}} d x . \quad(a>0)$
...
（3）若 $\varphi(u)$ 连续，则 $\int_{-a}^{a} \varphi\left(x^{2}\right) d x=2 \int_{0}^{a} \varphi\left(x^{2}\right) d x$例 8 计算 $\int_{-1}^{1} \frac{2 x^{2}+x \cos x}{1+\sqrt{1-x^{2}}} d x$ ．

[^0]例9 证明（1） $\int_{0}^{\frac{\pi}{2}} f(\sin x) d x=\int_{0}^{\frac{\pi}{2}} f(\cos x) d x$ ．
...
$$
\begin{aligned}
& \because f(x)=\sqrt{\sin ^{3} x-\sin ^{5} x}=\left\lvert\, \cos x(\sin x)^{\frac{3}{2}}\right. \\
& \therefore \int_{0}^{\pi} \sqrt{\sin ^{3} x-\sin ^{5} x} d x=\int_{0}^{\pi} \left\lvert\, \cos x(\sin x)^{\frac{3}{2}} d x\right. \\
...
= & \frac{1}{2} \cdot \frac{\pi}{2}+\frac{1}{2}[\ln \mid \sin t+\cos t]_{0}^{\frac{\pi}{2}}=\frac{\pi}{4} .
\end{aligned}
$$
...
例14 计算 $\int_{0}^{\frac{1}{2}} \arcsin x d x$ ．例15 计算 $\int_{0}^{\frac{\pi}{4}} \frac{x d x}{1+\cos 2 x}$ ．
例16计算 $\int_{0}^{\mathbf{1}} \frac{\ln (\mathbf{1}+\boldsymbol{x})}{(\mathbf{2}+\boldsymbol{x})^{\mathbf{2}}} \boldsymbol{d} \boldsymbol{x}$ ．例17 设 $f(x)=\int_{1}^{x^{2}} \frac{\sin t}{t} d t$ ，求 $\int_{0}^{1} x f(x) d x$ ．
...
& =\int_{\sqrt{e}}^{e^{\frac{3}{4}}} \frac{d(\ln x)}{\sqrt{\ln x} \sqrt{(1-\ln x)}}=2 \int_{\sqrt{e}}^{e^{\frac{3}{4}}} \frac{d \sqrt{\ln x}}{\sqrt{1-(\sqrt{\ln x})^{2}}} \\
& =2[\arcsin (\sqrt{\ln x})]_{\sqrt{e}}^{e^{\frac{3}{4}}}=\frac{\pi}{6}
解 原式 $=\int_{-1}^{1} \underset{\substack{\text { 偶函数 } \\ \frac{\mathbf{2} \boldsymbol{x}^{\mathbf{2}}}{\mathbf{1 + \sqrt { \mathbf { 1 } - \boldsymbol { x } ^ { \mathbf { 2 } } }}}}}{ } d x+\int_{-\mathbf{1}}^{1} \underset{\substack{\mathbf{1}+\sqrt{\mathbf{1}-\boldsymbol{x}^{\mathbf{2}}} \\ \text { 奇函数 }}}{\frac{\boldsymbol{x} \boldsymbol{\operatorname { c o s }} \boldsymbol{x}}{\mathbf{1}+}} d \boldsymbol{x}$
& =4-\pi .
...
\therefore & \int_{a}^{b} u d v=[u v]_{a}^{b}-\int_{a}^{b} v d u .
## 定积分的分部积分法习例
= & -\frac{\ln 2}{3}+\int_{0}^{1} \frac{1}{2+x} \cdot \frac{1}{1+x} d x \longrightarrow \frac{1}{1+x}-\frac{1}{2+x} \\
& =(n-1) I_{n-2}-(n-1) I_{n} \quad \therefore I_{n}=\frac{n-1}{n} I_{n-2}
## 内容小结
> 换元必换限配元不换限边积边代限
