...
例3． $\int\left(10^{x} \cdot 3^{2 x}+3 \sin x+\sqrt{x}\right) d x$
例4．计算 $\int \frac{(2 x \sqrt{x}-x+3 \sqrt{x}) \sqrt{x}}{x^{2}} d x \quad$

例5 $\int\left(2^{x}-3^{x}\right)^{2} d x$
例6． $\int\left(\frac{3}{1+x^{2}}-\frac{2}{\sqrt{1-x^{2}}}\right) d x$
...
例10． $\int \sec x(\sec x-\tan x) d x \quad$

例11． $\int \cot ^{2} x d x$ ．
例12．计算 $\int \frac{\cos 2 x}{\cos ^{2} x \sin ^{2} x} d x \quad$

例13． $\int \frac{1}{\sin ^{2} \frac{x}{2} \cos ^{2} \frac{x}{2}} d x$
...
（6） $\int \cos x d x=\sin x+C$ ；
（7） $\int \sin x d x=-\cos x+C$ ；
（8） $\begin{aligned} \int \frac{\boldsymbol{d x}}{\boldsymbol{\cos}^{2} \boldsymbol{x}} & =\int \sec ^{2} x d x \\ & =\tan x+C ;\end{aligned}$
