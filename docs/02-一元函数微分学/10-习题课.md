...
设函数 $f(x)$ 在区间 $(a, b)$内具有连续导数。基点： $\boldsymbol{A}\left(\boldsymbol{x}_{\mathbf{0}}, \boldsymbol{y}_{\mathbf{0}}\right)$ ， $M(x, y)$ 为曲线上任意一点，
...
$$
\begin{aligned}
& \left(\frac{\Delta s}{\Delta x}\right)^{2}=\left(\frac{\overparen{M}^{\prime}}{\Delta x}\right)^{2}=\left(\frac{\overparen{M^{\prime}}}{\left|M M^{\prime}\right|}\right)^{2} \cdot \frac{\left|M^{\prime} M^{\prime}\right|^{2}}{(\Delta x)^{2}} \\
...
& \therefore \lim _{\Delta x \rightarrow 0} \frac{\Delta s}{\Delta x}= \pm \lim _{\Delta x \rightarrow 0} \sqrt{\left(\frac{\overparen{M M}^{\prime}}{\left|M M^{\prime}\right|}\right)^{2} \cdot\left(1+\left(\frac{\Delta y}{\Delta x}\right)^{2}\right)}= \pm \sqrt{1+\left(\frac{d y}{d x}\right)^{2}}
\end{aligned}
$$
...
例2．设有曲线 $r=r(\theta)$ ，求 $d s$ ．
解：$\because\left\{\begin{array}{l}x=r(\theta) \cos \theta \\ y=r(\theta) \sin \theta\end{array}\right.$,
...
定义：设 $\overparen{M}^{\prime}=\Delta s$ ，由 $M$ 到 $M^{\prime}$ 的切线转角为 $\Delta \alpha$ ，
（1）$K=\left|\frac{\Delta \alpha}{\Delta s}\right|$ 称为平均曲率；
...
\therefore d \alpha & =\frac{y^{\prime \prime}}{1+y^{\prime 2}} d x . \quad \therefore \quad K=\left|\frac{d \alpha}{d s}\right|=\frac{\left|y^{\prime \prime}\right|}{\left(1+y^{\prime 2}\right)^{3 / 2}} .
\end{aligned}
$$
...
半径为 $\boldsymbol{a}$ 的圆周沿直线无滑动地滚动时，其上定点 $M$的轨迹即为摆线。
...
3．曲率圆
曲率圆 $\quad R=\frac{1}{K}=\frac{\left(1+y^{\prime 2}\right)^{3 / 2}}{\left|y^{\prime \prime}\right|}$
...
& =\lim _{x \rightarrow 0} \frac{1}{-\frac{3 \sin x}{x}-6 e^{x^{2}}-\left(\cos x+2 e^{x^{2}}+4 x^{2} e^{x^{2}}\right)}=-\frac{1}{12} .
\end{aligned}
$$

解2 $\because \sqrt{1+x^{2}}=\left(1+x^{2}\right)^{\frac{1}{2}}=1+\frac{1}{2} x^{2}+\frac{1}{2!} \frac{1}{2}\left(\frac{1}{2}-1\right) x^{4}+o\left(x^{4}\right)$
...
& \text { 当 } x=0 \text { 时, } g^{\prime}(0)=\lim _{x \rightarrow 0} \frac{g(x)-g(0)}{x}=\lim _{x \rightarrow 0} \frac{\frac{f(x)}{x}-f^{\prime}(0)}{x} \\
...
& \therefore g^{\prime}(x)= \begin{cases}\frac{x f^{\prime}(x)-f(x)}{x^{2}}, & x \neq 0 \\
...
例8 设 $f(x)$ 在 $[0,2]$ 上连续，在 $(0,2)$ 内可导，且 $f(2)=5 f(0)$ ，证明存在 $\xi \in(0,2)$ ，使得 $\left(1+\xi^{2}\right) f^{\prime}(\xi)=2 \xi f(\xi)$ ．
证 设 $F(x)=\frac{f(x)}{1+x^{2}}$ ，则 $F(x)$ 在 $[0,2]$ 上连续，在 $(0,2)$ 内可导，且 $F(0)=f(0), F(2)=\frac{f(2)}{5}=f(0)=F(0)$
由Rolle定理，至少存在一点 $\xi \in(0,2)$ 使得 $F^{\prime}(\xi)=0$
即 $F^{\prime}(\xi)=\left.\frac{\left(1+x^{2}\right) f^{\prime}(x)-2 x f(x)}{\left(1+x^{2}\right)^{2}}\right|_{x=\xi}=0 \left(1+\xi^{2}\right) f^{\prime}(\xi)-2 \xi f(\xi)=0, \therefore\left(1+\xi^{2}\right) f^{\prime}(\xi)=2 \xi f(\xi)$.
